...
export function AuthModal() {
  ...
  const handleEmail = async () => {
    ...
    if (error) {
      ...
      setMsg({ kind: 'ok', text: '注册成功，已自动登录' })
    ...
    <Modal open={open} onClose={close}>
      ...
      <div className="gg-modal-body">
        <div className="gg-subtabs gg-subtabs-full" role="tablist">
          <button type="button" className={`gg-subtab ${mode === 'login' ? 'gg-subtab-active' : ''}`} onClick={() => { setMode('login'); setMsg(null) }}>登录</button>
          <button type="button" className={`gg-subtab ${mode === 'register' ? 'gg-subtab-active' : ''}`} onClick={() => { setMode('register'); setMsg(null) }}>注册</button>
        </div>
